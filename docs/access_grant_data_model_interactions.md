...
**The overriding mechanism, precisely:** at vault-creation time, `CreateVault.vue` fetches `GET /api/settings` and uses `enableAutomaticAccessGrant`/
...
sequenceDiagram
    ...
    Web ->> HubBackend: PUT /api/vaults/{vaultId}/users/{userId} (or .../groups/{groupId})<br/>— RBAC membership only, no key yet
    Web ->> HubBackend: GET /api/vaults/{vaultId}/users-requiring-access-grant
    ...
    Web ->> HubBackend: POST /api/vaults/{vaultId}/access-tokens {userId: jwe, ...}
...
`PUT /api/vaults/{vaultId}/users/{userId}`/`.../groups/{groupId}`) — adding a user or group to a vault's membership is done through the web frontend regardless of which
...
sequenceDiagram
    ...
    loop background long-poll
        Web ->> HubBackend: GET /api/vaults/users-requiring-access-grant?wait=25
    ...
    alt not enabled
        ...
        loop each candidate user
            Web ->> HubBackend: GET /api/users/trusted/{userId} → signatureChain
        ...
        Web ->> HubBackend: POST /api/vaults/{vaultId}/access-tokens/auto {userId: jwe, ...}
...
- **Gating source.** Both read `automaticAccessGrant.enabled`/`trustThreshold` exclusively from the vault's own decrypted UVF metadata, never from the hub-wide
  `/api/settings` default (which is only ever a one-time seed at vault creation —
...
- **Older endpoint shape.** The desktop client still polls the older, per-vault surface (`GET /api/vaults/{vaultId}/users-requiring-access-grant` + generic
  `POST .../access-tokens`) instead of the newer global long-poll (`GET /api/vaults/users-requiring-access-grant?wait=`) and dedicated `POST .../access-tokens/auto`
...
- **Same source of truth.** Desktop (`HubUVFVaultProvider.create()` → `HubStorageLocationService.toPayload()`) and the web frontend (`CreateVault.vue`) both
  call the identical endpoint — `GET /api/settings` — and seed the new vault's `automaticAccessGrant.enabled`/`trustThreshold` straight from
  ...
  singleton `Settings` row, whatever an admin has currently set it to via `PUT /api/settings`.
...
- **Net effect:** a vault created by either client the moment before an admin changes `/api/settings` gets the old value; the moment after, both get the new value —
...
- `backend/src/main/java/org/cryptomator/hub/api/SettingsResource.java` — `GET`/`PUT /api/settings`, role gating, audit logging
