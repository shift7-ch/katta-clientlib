...
public class HubUVFVault extends CryptoVault {
    ...
    public HubUVFVault create(final Session<?> session, final String region, final VaultMetadataProvider metadata) throws BackgroundException {
        try {
            ...
            final UVFKeySet keys = provider.getKeys();
            ...
            final VaultDto vaultDto = new VaultDto()
                    ...
                    .uvfKeySet(UVFKeySet.serializePublicRecoveryKey(keys.recoveryKey()).toString());
            // Create vault in Hub
            ...
            vaultResourceApi.apiVaultsVaultIdAccessTokensPost(vaultId,
                    Collections.singletonMap(userDto.getId(), new UVFAccessTokenPayload(keys.memberKey(), keys.recoveryKey())
                            .encryptForUser(userKeys.ecdhKeyPair().getPublic())));
